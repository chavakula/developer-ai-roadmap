...
print("Macro F1 :", round(f1_score(gold, preds, average="macro"), 2))
...
Imagine you just joined **OrbitMart**, an online electronics shop. Every day, hundreds of customer emails arrive:
...
Today, a human reads each email and decides which team to send it to (orders, returns, tech support, billing). Slow and expensive.

**Your job in this chapter:** teach a small AI model to read each email and pick the right team automatically.

That tiny task is small enough to actually finish, but **every** big idea in modern AI shows up while you build it:

| Step in the OrbitMart task | The big AI idea it teaches you |
|---|---|
| Splitting the words of an email into pieces | **Tokenization** |
| Turning each word into numbers the computer can use | **Embeddings** |
| Letting the model make a guess | **Forward pass + logits** |
| Measuring how wrong the guess was | **Loss** |
| Nudging the model to be less wrong next time | **Backpropagation + optimizer** |
| Doing this for many emails | **Batches and epochs** |
| Checking if the model actually works on **new** emails | **Train / validation / test split** |
| Going beyond "is it accurate?" | **Precision, recall, F1, confusion matrix** |

Then, in the second half of the chapter, we change the task. Instead of *classifying* emails, we ask the model to **finish** sentences (like ChatGPT does). Same pieces — only the goal changes.

By the end you should be able to look at any modern training script and say: *"OK, I see — that's just tokenization, embedding, forward pass, loss, backward, optimizer, repeat."*
...
- how tokens become tensors (the email words → numbers step)
- where loss is computed (the "how wrong was the guess?" step)
- what a batch really is (how many emails we show the model at once)
- why train/validation/test splits matter (how we avoid fooling ourselves)
- how model behavior is evaluated (going beyond accuracy)
- the difference between **causal language modeling** and **masked language modeling** (writing vs filling-in-the-blank)
...
Before diving into terms, here is the **whole pipeline you'll build** in one picture, told as the OrbitMart story:

```text
"Where is my package?"          ← the customer's email (raw text)
        │
        ▼ tokenizer
[Where, is, my, package, ?]     ← chopped into pieces (tokens)
        │
        ▼ vocabulary lookup
[42, 18, 7, 233, 9]             ← each piece gets a number (token IDs)
        │
        ▼ embedding layer
[[0.1, -0.4, ...],              ← each number becomes a list of numbers
 [0.3,  0.2, ...],                (an "embedding" — captures meaning)
 ...]
        │
        ▼ model layers (transformer / RNN / etc.)
[[...processed...],             ← the model "thinks" about the words
 [...processed...], ...]
        │
        ▼ classifier head
[2.1, -0.5, 0.3, 1.8, -1.2, 0.9]  ← raw scores, one per team (logits)
        │
        ▼ pick the highest
"order_status"                  ← the predicted team
```

That is **the entire chapter in one diagram**. The next ~50 small sections each zoom into one box.

A modern text model pipeline is, in shorthand:
